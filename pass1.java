...
class pass1
{
    ...
    {
        FileReader FP=new FileReader("D:/input.txt");   //byte reader
        BufferedReader bufferedReader = new BufferedReader(FP);   //buffer number of bytes & store into buffer ,read  character

        String line=null;
        int line_count=0,LC=0,symTabLine=0,opTabLine=0,litTabLine=0,poolTabLine=0;  //declared the varaiable

        //Data Structures
        final int MAX=100; //location
        String SymbolTab[][]=new String[MAX][3];  // creating the table
        ...
        while((line = bufferedReader.readLine()) != null) //reading perticular line
        {
            String[] tokens = line.split("\t");
            if(line_count==0)
            {
                LC=Integer.parseInt(tokens[1]);    //start 100
